...
def detectDuplicatesInBlock(block: TealBlock) -> TealBlock:
    ...
    while index > 0:
        ...
        if currDeps == prevDeps:
            ...
            block.ops = block.ops[:index - len(currDeps) + 1] + [TealOp(Op.dup)] + block.ops[index + 1:]
            index -= len(currDeps)
            continue
        ...
        if prevCurrIndex > 0:
            ...
            if prevCurrComplete and prevPrevIndex >= 0 and currDeps == prevCurrDeps:
                ...
                if prevPrevComplete and prevPrevDeps == prevDeps:
                    ...
                    block.ops = block.ops[:prevCurrIndex + 1] + [TealOp(Op.dup2)] + block.ops[index + 1:]
